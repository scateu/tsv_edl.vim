...
function! tsv_edl#ipc_init_and_load_media(...) "pause = v:true)
	...
	if g:cherry_pick_mode_entered
		echon "already in cherry_pick_mode, will not map Enter to Seek"
	else
...
function! tsv_edl#ipc_toggle_play()
	...
	if result ==? "true"
		...
		if g:PlayheadSeekTimerId != -1  "has a running seek timer, stop it.
			call timer_pause(g:PlayheadSeekTimerId, 0) " resume timer
		endif
	elseif result ==? "false"
		call tsv_edl#ipc_always_pause()
		if g:PlayheadSeekTimerId != -1  "has a running seek timer, stop it.
			call timer_pause(g:PlayheadSeekTimerId, 1) " resume timer
		endif
	endif
endfunction
...
function! tsv_edl#write_record_out(...)
	...
	elseif (getline(".") =~# pattern_2) "full line overwrite
		...
		call setline('.', origin_line_first_part . rec_out . "\t" . '| ' . g:ipc_loaded_media_name . ' |' . "\t" . origin_line_second_part)
	endif
	call cursor(0,col('$'))
	"startinsert!
	return playback_time
