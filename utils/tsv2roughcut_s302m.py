...
def handle_local_clip(f, r_in, r_out, f_B, counter, tempdirname):
    ...
    if ext in video_formats:
        subprocess.call("ffmpeg -hide_banner -loglevel error -ss %s -i \"%s\" -ss %s -to %s -vf 'fps=24, scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,setsar=1' -c:v %s -b:v 2M %s/%05d.%s"%(t2, f, t3, to, codec_v, tempdirname,counter, fragment_ext), shell=True)
    ...
    handle_b_roll(f_B, tempdirname, counter, fragment_ext, codec_v, codec_a)
...
######### 2. stitch lines
    ...
    #eprint(output_queue);sys.exit(-1)
...
########## 3.2 video project
        ...
        with tempfile.TemporaryDirectory() as tempdirname:
            ...
            subprocess.call("ffmpeg -hide_banner -loglevel error -safe 0 -f concat -i %s/roughcut.txt -strict -2 -ss 0 -c:v copy  %s"%(tempdirname, roughcut_filename), shell=True)
            #TODO if has B roll, add acrossover or adeclick here.
