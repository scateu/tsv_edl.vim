...
def handle_local_clip(f, r_in, r_out, f_B, counter, tempdirname):
    ...
    if ext in video_formats:
        subprocess.call("ffmpeg -hide_banner -loglevel error -ss %s -i \"%s\" -ss %s -to %s -vf 'fps=24, scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,setsar=1' -c:v %s -b:v 2M -shortest %s/%05d.ts"%(t2, f, t3, to, codec_v, tempdirname,counter), shell=True)
...
def process_edl_input():
    ...
    return output_queue, srt_queue

def handle_audio_project(output_queue, srt_queue, args):
    ...
    return roughcut_filename, srt_filename, roughcut_ext_name

def handle_video_project(output_queue, srt_queue, args):
    roughcut_txt_lines = []
    with tempfile.TemporaryDirectory() as tempdirname:
        eprint("[tempdir]", tempdirname)
        counter = 0
...
def handle_file_renaming(roughcut_filename, srt_filename, roughcut_ext_name, args):
...
if __name__ == "__main__":
    ...
    output_queue, srt_queue = process_edl_input()

    if is_pure_audio_project:
        roughcut_filename, srt_filename, roughcut_ext_name = handle_audio_project(output_queue, srt_queue, args)
    else:
        roughcut_filename, srt_filename, roughcut_ext_name = handle_video_project(output_queue, srt_queue, args)

    roughcut_filename, srt_filename = handle_file_renaming(roughcut_filename, srt_filename, roughcut_ext_name, args)
