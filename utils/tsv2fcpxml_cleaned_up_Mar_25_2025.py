...
def stitch_fcpxml_queue(raw_queue):
    #([clipname, ref_id, offset, fcpx_record_in, duration, lane, subtitle ])
...
def process_edl_input(FPS, FCPX_SCALE, OFFSET_1HOUR, GENERATE_SRT):
    ...
    return media_assets, output_queue, output_queue_B, srt_queue

def stitch_queues(output_queue, output_queue_B):
    ...
    return output_queue, output_queue_B

def generate_xml_header(media_assets, FPS, FCPX_SCALE):
    xmlhead = ""
...
def generate_xml_body(output_queue, output_queue_B, media_assets, FCPX_SCALE):
    xmlbody = ""
...
def write_srt_file(srt_queue, GENERATE_SRT):
    if GENERATE_SRT:
...
if __name__ == "__main__":
    ...
    # Process input
    media_assets, output_queue, output_queue_B, srt_queue = process_edl_input(FPS, FCPX_SCALE, OFFSET_1HOUR, GENERATE_SRT)
    
    # Stitch queues
    output_queue, output_queue_B = stitch_queues(output_queue, output_queue_B)
    
    # Generate XML
    xmlhead = generate_xml_header(media_assets, FPS, FCPX_SCALE)
    xmlbody = generate_xml_body(output_queue, output_queue_B, media_assets, FCPX_SCALE)
    ...
    # Write SRT file if needed
    write_srt_file(srt_queue, GENERATE_SRT)
